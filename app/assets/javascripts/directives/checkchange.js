...
var viewModelDateComparison = function(scope, ctrl) {
  viewValueDate = new Date(ctrl.$viewValue);
  viewValueDateCmp = viewValueDate.getUTCMonth() + 1 + "/" + viewValueDate.getUTCDate() + "/" + viewValueDate.getUTCFullYear();

  modelCopyDate = new Date(scope.modelCopy[ctrl.$name]);
  modelCopyDateCmp = modelCopyDate.getUTCMonth() + 1 + "/" + modelCopyDate.getUTCDate() + "/" + modelCopyDate.getUTCFullYear();
...
var checkForOverallFormPristinity = function(scope, ctrl) {
  modelCopyObject = JSON.parse(JSON.stringify(scope.modelCopy));
  delete modelCopyObject[ctrl.$name];

  modelObject = JSON.parse(JSON.stringify(scope[scope.model]))
