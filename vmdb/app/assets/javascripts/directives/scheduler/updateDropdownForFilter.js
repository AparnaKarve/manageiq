miqAngularApplication.directive('updateDropdownForFilter', function() {
  return {
    require: 'ngModel',
    link: function (scope, elem, attr, ctrl) {
      ...
      scope.$watch(scope['form_' + ctrl.$name + '_dropdownList'], function() {
        selectListElement(scope, ctrl, true);
      });

      ctrl.$parsers.push(function(value) {
        if(scope.invalidStyleSet) {
          $(scope['form_' + ctrl.$name]).selectpicker('setStyle', 'btn-primary', 'remove');
...
var selectListElement = function(scope, ctrl, refresh) {
  setTimeout(function(){
    if(refresh) {
      if (scope[scope['form_' + ctrl.$name + '_ngHide']] === true) {
        $(scope['form_' + ctrl.$name]).selectpicker('hide');
    ...
    if (scope[scope['form_' + ctrl.$name + '_dropdownModel']][ctrl.$name] != undefined &&
      ...
      if(index == null) {
        $(scope['form_' + ctrl.$name]).selectpicker('setStyle', 'btn-primary');
