class ProviderForemanController < ApplicationController
  ...
  def delete
    ...
    if foremen.empty?
      ...
      ConfigurationManagerForeman.find_all_by_id(foremen, :order => "lower(name)").each do |foreman|
        id = foreman.id
      ...
      ConfigurationManagerForeman.destroy_queue(foremen)
  ...
  def foreman_providers_tree_rec
    ...
    when "e"    then  rec = identify_record(params[:id], ConfigurationProfile)
    when "cp"   then  rec = identify_record(params[:id], ConfiguredSystem)
    end
    rec
  end
